...
const Sidebar = () => {
    ...
    border-slate-100 dark:border-slate-800 p-6 transition-colors duration-200">
            ...
            <div className="flex flex-col gap-2">
                {navItems.map((item) => (
                    <NavLink key={item.path} to={item.path}
                        className={({ isActive }) => `flex items-center gap-3 px-4 py-2.5 border-l-4 transition-all duration-200
                       ${isActive ? 'bg-amber-50 dark:bg-amber-900/10 text-emerald-600 dark:text-emerald-400 font-medium' :
            ...
            <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800">
                ...
                hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-700 dark:hover:text-slate-200 rounded-lg transition-colors
